...
export default function RequestsView({ showToast }) {
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([
      requestsApi.getIncoming().catch(() => { showToast('Failed to load incoming'); return [] }),
      requestsApi.getOutgoing().catch(() => { showToast('Failed to load sent'); return [] })
    ])
    .then(([inc, out]) => {
      setIncoming(inc)
      setOutgoing(out)
  ...
  const handleAction = async (id, action) => {
    try {
      ...
      setIncoming(prev => prev.filter(r => r.requestId !== id))
    } catch (err) {
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        {incoming.map(req => (
          <div key={req.requestId} className="request-card" style={{ 
            display: 'grid', gridTemplateColumns: 'min-content 1fr min-content', gap: '32px', border: '1px solid var(--line)', background: 'var(--ink)', padding: '32px', borderRadius: '2px'
          }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div>
                ...
                <p style={{ color: 'var(--paper-dim)', margin: 0, fontFamily: 'var(--font-serif)' }}>{req.sender.branch || 'Unknown branch'} • {req.sender.year || 'Unknown year'}</p>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {req.sender.skills.map(s => <span key={s} style={{ padding: '4px 10px', background: 'var(--surface)', color: 'var(--paper)', fontSize: '0.8rem', borderRadius: '4px' }}>{s}</span>)}
                  </div>
                </div>
                <div>
                  <strong style={{ fontSize: '0.65rem', letterSpacing: '0.15em', color: 'var(--muted)', display: 'block', marginBottom: '8px' }}>LOOKING FOR</strong>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {req.sender.lookingFor.map(s => <span key={s} style={{ padding: '4px 10px', border: '1px solid var(--line)', color: 'var(--paper-dim)', fontSize: '0.8rem', borderRadius: '4px' }}>{s}</span>)}
                  </div>
                </div>
              </div>
  ...
  const renderSent = () => {
    if (outgoing.length === 0) {
      return (
        <div className="status-empty">
          <p className="eyebrow">NO SENT REQUESTS</p>
          <h2>You haven't requested to team up with anyone yet.</h2>
        </div>
      )
    }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        {outgoing.map(req => (
          <div key={req.requestId} className="request-card" style={{ 
            ...
            <div style={{ width: '120px' }}>
              <AnimalAvatar animal={req.receiver.avatar || 'raccoon'} label={req.receiver.name} />
            </div>
            
            <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
              <div>
                <h3 style={{ fontSize: '1.8rem', margin: '0 0 8px 0', lineHeight: 1 }}>{req.receiver.name}</h3>
                <p style={{ color: 'var(--paper-dim)', margin: 0, fontFamily: 'var(--font-serif)' }}>{req.receiver.branch || 'Unknown branch'} • {req.receiver.year || 'Unknown year'}</p>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
                <div>
                  <strong style={{ fontSize: '0.65rem', letterSpacing: '0.15em', color: 'var(--muted)', display: 'block', marginBottom: '8px' }}>THEIR SKILLS</strong>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {(req.receiver.skills || []).map(s => <span key={s} style={{ padding: '4px 10px', background: 'var(--surface)', color: 'var(--paper)', fontSize: '0.8rem', borderRadius: '4px' }}>{s}</span>)}
                  </div>
                </div>
                <div>
                  <strong style={{ fontSize: '0.65rem', letterSpacing: '0.15em', color: 'var(--muted)', display: 'block', marginBottom: '8px' }}>LOOKING FOR</strong>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {(req.receiver.lookingFor || []).map(s => <span key={s} style={{ padding: '4px 10px', border: '1px solid var(--line)', color: 'var(--paper-dim)', fontSize: '0.8rem', borderRadius: '4px' }}>{s}</span>)}
                  </div>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="content-section" style={{ maxWidth: '960px', margin: '0 auto', padding: '40px 24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px' }}>
        <h2 style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', margin: 0 }}>Team Requests</h2>
        <div style={{ display: 'flex', gap: '16px', borderBottom: '1px solid var(--line)' }}>
          <button 
            onClick={() => setTab('incoming')} 
            style={{ 
              background: 'transparent', border: 'none', 
              color: tab === 'incoming' ? 'var(--accent)' : 'var(--paper-dim)', 
              borderBottom: tab === 'incoming' ? '2px solid var(--accent)' : '2px solid transparent',
              padding: '8px 16px', cursor: 'pointer', fontWeight: 'bold', letterSpacing: '0.1em'
            }}
          >
            INCOMING
          </button>
          <button 
            ...
            style={{ 
              background: 'transparent', border: 'none', 
              color: tab === 'sent' ? 'var(--accent)' : 'var(--paper-dim)', 
      ...
      {loading ? <div>Loading...</div> : (tab === 'incoming' ? renderIncoming() : renderSent())}
    </div>
  )
}
